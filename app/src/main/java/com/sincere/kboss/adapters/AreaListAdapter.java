...
public class AreaListAdapter extends BaseAdapter {
    ...
    ArrayList<STWorkingArea> m_areas = null;
    ...
    public AreaListAdapter(Activity activity, Context context) {
        mContext = context;
        mActivity = activity;
    ...
    public void setData(ArrayList<STWorkingArea> areas)
    {
        boolean allfalse = false;
        m_areas = areas;

        String prefer_zone = KbossApplication.g_userinfo.f_prefer_zone;
        if (prefer_zone.equals("0")) {
            allfalse = true;
        }
        ...
        if (allfalse == false) {
            try {
                String[] areaids = prefer_zone.split(",");
                for (int i = 0; i < areaids.length; i++) {
                    int areaid = Integer.parseInt(areaids[i]);
                    m_chkStats.set(areaid, true);
                }
            }catch(Exception e){}
        }
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        try
        {
            STWorkingArea  anItem = null;
            if (position > 0)
            {
                anItem = m_areas.get(position-1);
            }
            ...
            if (KbossApplication.UI_TEST == false) {
                ...
                if (position == 0) {
                    lblAreaName.setText(mActivity.getString(R.string.personalinfosetting_wholecount));
                    chkArea.setOnCheckedChangeListener(checkedAllChangeListener);
                } else {
                    lblAreaName.setText(anItem.f_area);
    ...
    public String getWorkingAreaString() {
